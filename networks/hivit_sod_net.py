...
class HiViTSOD(nn.Module):
    def __init__(self, img_size = (512, 1024)):
        ...
        self.equi_encoder =  HiViT(img_size=224,  patch_size=16, inner_patches=4, in_chans=3, num_classes=1000,
                                   ...
                                   norm_layer=partial(nn.LayerNorm, eps=1e-6), ape=True, rpe=True, patch_norm=True, use_checkpoint=True,
    ...
    def forward(self, samples, samples_hivit, pers, xyz, uv, center_points):
        ...
        for i in range(len(feats)):
            ...
            p2e_feat[i]  = pers2equi(p2e_feat[i], nrows=3, fov=(120, 120), patch_size=patch_size, erp_size=erp_size, layer_name=layer_name)
        ...
        # 频域傅里叶卷积
        equi_feat_list[0] = self.fsfc0(equi_feat_list[0])
        equi_feat_list[1] = self.fsfc1(equi_feat_list[1])
        equi_feat_list[2] = self.fsfc2(equi_feat_list[2])
        ...
        # ============== ERP、E2P 交互 ==============
        ep_feat0 = self.epfusion0(equi_feat_list[0], p2e_feat[2])
        ep_feat1 = self.epfusion1(equi_feat_list[1], p2e_feat[3])
        ep_feat2 = self.epfusion2(equi_feat_list[2], p2e_feat[4])
        ep_feat_list = [ep_feat0, ep_feat1, ep_feat2]
        
        # ============== 解码器 ==============
        feat5_pred = self.rgb_global( p2e_feat[-1])
        preds = []
        preds = [
            torch.sigmoid(
                F.interpolate(feat5_pred,
                              size=(H_e, W_e),
                              mode='bilinear',
                              align_corners=True)),
        ]
        ...
        for idx in [4, 3, 2, 1, 0]:
            if idx>=2:
                _p = self.bams[idx](ep_feat_list[idx-2], p, uncertain=sat_uncer_list[idx-2])
            else:
                _p = self.bams[idx](p2e_feat[idx], p)
    ...
    def load_pre(self, pre_model):
        ...
        self.equi_encoder.load_state_dict({k: v for k, v in trained_model_state_dict.items() if k in model_state_dict},strict=False)
